...
public abstract class AbstractQueuedSynchronizer
    ...
    private Node enq(final Node node) {
        for (;;) {
            //Node t=tail=head=new Node();
            Node t = tail;
            if (t == null) { // Must initialize
                if (compareAndSetHead(new Node()))
                    tail = head;
            } else {
                //tail=node;tail.prve=head;
                node.prev = t;
                //tail=node;
                if (compareAndSetTail(t, node)) {
                    //
                    //t=head; head.next=tail;
                    t.next = node;
                    return t;
                }
            }
    ...
    public final boolean hasQueuedPredecessors() {
        ...
        Node h = head;
        Node s;
        //如果头结点与尾节点不相同并且头结点的下一个节点为空或者不为空并且下一点节点线程不是当前线程则为true,否则为false
        /**
         *
         * @<code>
         *
         *  private Node addWaiter(Node mode) {
                ...
                if (pred != null) {
                    node.prev = pred;
                    if (compareAndSetTail(pred, node)) {//(s = h.next) == null 表示这段代码,第一次尝试获取锁失败,就需要添加至等待队列
                        pred.next = node;//s.thread != Thread.currentThread() 表示(s = h.next) !=null,并且不是当前线程
                        return node;
                    }
                }
                enq(node);
    ...
    final boolean transferForSignal(Node node) {
        ...
        Node p = enq(node);
        int ws = p.waitStatus;
        //如果当前节点上一节点取消或者修改上一节点状态为等待通知锁失败,则唤醒当前节点
